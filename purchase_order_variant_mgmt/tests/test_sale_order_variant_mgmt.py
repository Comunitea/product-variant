...
class TestSaleOrderVariantMgmt(common.SavepointCase):
    ...
    def test_add_variants(self):
        ...
        wizard.variant_line_ids[0].product_qty = 1
        wizard.variant_line_ids[1].product_qty = 2
        wizard.variant_line_ids[2].product_qty = 3
        wizard.variant_line_ids[3].product_qty = 4
        wizard.button_transfer_to_order()
        self.assertEqual(len(self.order.order_line), 4,
                         "There should be 4 lines in the sale order")
        self.assertEqual(self.order.order_line[0].product_qty, 1)
        self.assertEqual(self.order.order_line[1].product_qty, 2)
        self.assertEqual(self.order.order_line[2].product_qty, 3)
        self.assertEqual(self.order.order_line[3].product_qty, 4)
    ...
    def test_modify_variants(self):
        ...
        wizard._onchange_product_tmpl_id()
        self.assertEqual(
            len(wizard.variant_line_ids.filtered('product_qty')), 2,
            "There should be two fields with any quantity in the wizard."
        )
        wizard.variant_line_ids.filtered(
            lambda x: x.product_id == product1).product_qty = 0
        wizard.variant_line_ids.filtered(
            lambda x: x.product_id == product2).product_qty = 10
        wizard.button_transfer_to_order()
        self.assertFalse(order_line1.exists(), "Order line not removed.")
        self.assertEqual(
            order_line2.product_qty, 10, "Order line not change quantity.")
